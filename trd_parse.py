...
def write_to_file(file_link, data, fieldnames):
    print('Начинаем запись договоров ГО(' + str(bin) + ') в файл')
    ...
    print('Завершена запись договоров ГО(' + str(bin) + ') в файл')
...
def proc_contracts(contracts):
    ...
    current_cont_num = 0
    print('Всего договоров в ГО' + '(' + str(bin) + '): ', contracts_count)
...
def get_contracts(supplier_biin, fin_year=2021, file_link='result/result.csv'):
    ...
    for biin in bin_list:
        ...
        while True:
            time.sleep(1)
            params = {"supplierBiin": biin, "after": after, "finYear": fin_year}
            try:
...
get_contracts(supplier_biin=read_bins(), file_link=create_file(FILE), fin_year=2018)
